...
Was sent to the following email addresses:<br/><ul>";


// Store a recipients variable.
$recipients = '';
...
// Work through all of the ticket applicants and decide if they need an email.
	...
	for ($j = 0; $j < $numrows; ++$j)
    {
		...
		$query = "SELECT id FROM pem_reservations WHERE mainemail='".$mainemail."' ORDER BY id ASC";
		...
		if ($checkbox == "checked") {
			...
			$recipients = $recipients.$mainemail.'<br/>';
		}
    };
